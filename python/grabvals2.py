...
os.chdir('..'); os.chdir('..'); os.chdir('Intermediate')
hsisdata = pd.read_csv('hsis_merged.csv', index_col = 'hsis_childid', na_values = '')
...
for var in sortedvars:
	...
	values = ' '.join([' '.join([v,'"' + valdict[v] + '"']) for v in values \
					if hsisdata[var][hsisdata[var] == float(v)].count() > 0])
	if values.strip() != '':
		string += 'label define ' + labels[var][1] + ' ' + values + ', replace' + '\n'
		string += 'label values ' + var + ' ' + labels[var][1] + '\n'
